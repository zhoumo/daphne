...
public class PdfUtil {
	...
	public static void createPdfTable(PdfPTable mainTable, ScrumStory story, Map<String, String> userNamesMap) throws Exception {
		...
		table.addCell(createPdfCell(story.getJiraKey(), 15f, BaseColor.LIGHT_GRAY));
		table.addCell(createPdfCell(story.getSummary(), 90f, BaseColor.WHITE));
		table.addCell(createPdfCell("研发: " + userNamesMap.get(story.getCodeTaker()) + ",  测试: " + userNamesMap.get(story.getTestTaker()), 15f, BaseColor.LIGHT_GRAY));
